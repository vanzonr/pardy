...
struct system_t {
    ...
    int       neighborrank[27]; ///< ranks of the neighbors. Entry 13
                                /// is this process, 13+/-1 is the
                                /// process to the forward or backward
                                /// in the x direction, 13+/-3 is the
                                /// process right or left in the y
                                /// direction, 13+/-9 is the process
                                /// up or down in the z direction.
    bool      neighborsendblock[27]; ///< which of the above ranks to
                                     /// skip when sending ghost
                                     /// particles, because it would
                                     /// deliver the same ghost data
                                     /// to the same process twice (or
                                     /// more). This can happen if the
                                     /// number of processors in a
                                     /// given dimension is two, and
                                     /// that direction only allows
                                     /// for 1 cell per
                                     /// processor. Likewise, if the
                                     /// number of processors in a
                                     /// given dimension is one, no
