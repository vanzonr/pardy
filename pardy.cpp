...
void initialize(rvector<atom_t>& atoms, interaction_pairs_t& p, system_t& sys, parallel_work_t& work)
{
    double scale;
    int i;
    /* generate positions */
...
void run(system_t& sys)
{
    ...
    const int maxsendcells = std::max({sys.nc[0]*sys.nc[1], sys.nc[0]*sys.nc[2], sys.nc[1]*sys.nc[2]});
