...
///
/// @brief Computes a super-cellindex out of the cell
/// indices cx, cy and cz. This super cell index is used to sort the
/// particles into cells, and as an ordering for cells. cx is assumed
/// to lie between minc[0] and minc[0]+nc[0], cy between minc[1] and
/// minc[1]+nc[1], and cz between minc[2] and minc[2]+nc[2].  The
/// equation is: super-index =
/// ((z-minc[2])*nc[1]+(y-minc[1]))*nc[0]+(x-minc[0]);
///
/// @param cx cell index in the x direction.
/// @param cy cell index in the y direction.
/// @param cz cell index in the z direction.
/// @param minc[] is an array of 3 elements which are the minimum cell indices in each direction.
/// @param nc[] is an array of 3 elements which are the number of cells in each direction.
///
/// @returns the super index
///
...
/// @param nc is an array of 3 elements which are the number of cells in each direction.
///
/// @returns the cell index in the x directions.
///
...
///
/// Find pairs with particles in neighboring cells given a cell.
///
...
///
/// @brief Find pairs with particles in neighboring cells given a set
/// of ghost particles.
///
/// @param p a reference to an interaction_pairs_t structure that will
///          hold the interaction pairs, i.e., in the indices of the
